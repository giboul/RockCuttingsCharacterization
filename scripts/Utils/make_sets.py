...
"""This script is a place for preparing the datasets (*.csv) in the /config/sets folder.
...
def create_dataset(to_folder, data_folders=None, keyword=""):
    ...
    Args:
        ...
        keyword: str="": filters folders to only thow whose name contains the keyword
    """
    ...
    print(f"{'Set':^{_max_len}} : {'Folder':^{_max_len1}} | {'Rock':^4} = {'label':^5}")
    paths = []
    ...
    for data_set in data_folders:
        if not data_set=='Raw':
            ...
            for folder in folders:
                # Rock type acronym should be the first two letters of the folder
                rock = folder[:2]
                if rock in label_dict:
                    label = label_dict[rock]
                    print(f"{data_set:^{_max_len}} : {folder:^{_max_len1}} | {rock:^4} = {label:^5}")
                    for file in listdir(path_to(data_folder, data_set, folder)):
...
def modify():
    ...
    for dir in [
        ...
        for file in listdir(path_to(data_folder, dir)):
            ...
            columns = [column for column in df.columns if not 'Unnamed' in column]
            df = df[columns]
            # Commented the save to make it safer (yo don't wanna overwrite everything)
            # Just uncomment the following line if it's all good to go
...
if __name__ == '__main__':
    ...
    # create_dataset(path_to('config', 'sets', 'debug', 'debug.csv'), keyword='debug')
    # modify()
